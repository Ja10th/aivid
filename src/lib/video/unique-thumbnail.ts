 ...
 * 1. extractHook now returns { text, specifics } — specifics is real structured
 *    data (the actual riddle question, the actual equation, the actual sequence,
 *    the actual true/false statements) that gets threaded all the way through to
 *    the generator functions instead of being discarded after routing.
 * 2. Generator functions that can use real content now accept it as an optional
 *    param and fall back to generic content only when the video genuinely has
 *    nothing specific to show.
 * 3. Added new grammar implementations so named variants in the pools actually
 *    produce distinct output instead of aliasing the same 4-5 functions.
 * 4. Added a hook-banner fallback so even an unmatched/default grammar renders
 *    something tied to this video's specific hook, not a generic template.
 */
...
export interface HookSpecifics {
  riddleQuestion?: string;
  riddleAnswer?: string;
  equation?: string;
  numMathProblems?: number;
  sequence?: number[];
  memoryLength?: number;
  statements?: string[];
  trueIndex?: number;
  gameType?: string;
  numScenes?: number;
  eyeExerciseTypes?: string[];
}

interface HookResult {
  text: string;
  specifics: HookSpecifics;
}

/**
 * Extract the unique hook from a video - what makes THIS video different from all others.
 * Returns both a human-readable summary (used for grammar routing) AND the raw
 * structured data (used for actually rendering real content in the thumbnail).
 */
function extractHook(comp: Composition): HookResult {
  const { meta, scenes, category } = comp;
  const title = meta?.title || "";
  const specifics: HookSpecifics = { numScenes: scenes?.length || 0 };
  ...
  const hasEyeTraining =
    category === "eye_training" ||
    sceneKinds.some((k) =>
      [
        "smooth-pursuit",
        "saccade",
  ...
  const riddleData = scenes?.find((s) => s.kind === "riddle")?.data as
    | { question?: string; answer?: string }
    | undefined;
  const mathData = scenes?.find((s) => s.kind === "math-question")?.data as
    | { q?: string }
    | undefined;
  const memoryData = scenes?.find((s) => s.kind === "memory-challenge")
    ?.data as { sequence?: number[] } | undefined;
  const mythData = scenes?.find((s) => s.kind === "myth-buster")?.data as
    | { statements?: string[]; trueIndex?: number }
    | undefined;
  const wyrData = scenes?.find((s) => s.kind === "would-you-rather")?.data as
    | { optionA?: string; optionB?: string }
    | undefined;

  if (hasEyeTraining) {
    specifics.eyeExerciseTypes = sceneKinds.filter((k) =>
      [
        "smooth-pursuit",
        "saccade",
        "convergence",
  ...
  if (hasRiddle && riddleData?.question) {
    specifics.riddleQuestion = riddleData.question;
    specifics.riddleAnswer = riddleData.answer;
    return {
      text: `A riddle that makes you think: ${riddleData.question.substring(0, 50)}...`,
      specifics,
    };
  }

  if (hasMath && mathData?.q) {
    specifics.equation = mathData.q;
    specifics.numMathProblems = scenes?.length || 0;
    return {
      text: `${specifics.numMathProblems} rapid-fire math problems - can you keep up?`,
      specifics,
    };
  }

  if (hasMemory && memoryData?.sequence) {
    specifics.sequence = memoryData.sequence;
    specifics.memoryLength = memoryData.sequence.length;
    return {
      text: `Remember ${specifics.memoryLength} items in sequence - harder than it looks`,
      specifics,
    };
  }

  if (hasWouldYouRather) {
    if (wyrData?.optionA && wyrData?.optionB)
      specifics.statements = [wyrData.optionA, wyrData.optionB];
    return {
      text: `Impossible choices that reveal who you really are`,
      specifics,
    };
  }

  if (hasMythBuster) {
    if (mythData?.statements) {
      specifics.statements = mythData.statements;
      specifics.trueIndex = mythData.trueIndex;
    }
    return {
      text: `Common beliefs debunked with science and facts`,
      specifics,
    };
  }
  ...
  if (hasPoll)
    return {
      text: `Your opinion matters - vote and see what others think`,
      specifics,
    };

  if (hasGame) {
    specifics.gameType =
      sceneKinds.find((k) => k.startsWith("game-"))?.replace("game-", "") ||
      "challenge";
    return {
      text: `Interactive ${specifics.gameType} game you can play along with`,
      specifics,
    };
  }
  ...
  if (titleLower.includes("impossible") || titleLower.includes("extreme")) {
    return {
      text: `The ultimate challenge - think you can handle it?`,
      specifics,
    };
  }
  if (titleLower.includes("brain") || titleLower.includes("iq")) {
    return {
      text: `Test your mental skills with these mind-bending challenges`,
      specifics,
    };
  }
  ...
  if (maxType && maxType[1] > 0) {
    return {
      text: `${maxType[1]} ${maxType[0]} challenges that will push your limits`,
      specifics,
    };
  }

  return {
    text: title || `A unique ${category} experience designed to challenge you`,
    specifics,
  };
}

/**
 * Choose a composition grammar based on the hook text. Unchanged in spirit from
 * before — keyword-routed into pools — but pools now include the new grammar
 * names added below, and each pool has more real (non-aliased) implementations.
 */
function chooseGrammar(
  hook: string,
  rng: RNG,
  excludeGrammars: string[] = [],
  variantIndex = 0,
): string {
  ...
  const pickGrammar = (options: string[]) => {
    ...
    return (
      available[(start + variantIndex) % available.length] ??
      rng.pick(available)
    );
  };

  if (
    hookLower.includes("eye") ||
    hookLower.includes("follow") ||
    hookLower.includes("track") ||
    hookLower.includes("focus") ||
    hookLower.includes("vision")
  ) {
    return pickGrammar([
      "concentric-target",
      "medical-diagram",
  ...
  if (
    hookLower.includes("riddle") ||
    hookLower.includes("question") ||
    hookLower.includes("think")
  ) {
    return pickGrammar([
      "giant-question-mark",
      "riddle-scroll",
  ...
  if (
    hookLower.includes("sequence") ||
    hookLower.includes("remember") ||
    hookLower.includes("memory") ||
    hookLower.includes("item")
  ) {
    return pickGrammar([
      "grid-faces",
      "sequence-beads",
  ...
  if (
    hookLower.includes("choice") ||
    hookLower.includes("rather") ||
    hookLower.includes("decide") ||
    hookLower.includes("reveal")
  ) {
    return pickGrammar(["vs-battle", "statement-lineup"]);
  }

  if (
    hookLower.includes("myth") ||
    hookLower.includes("debunk") ||
    hookLower.includes("fact") ||
    hookLower.includes("truth") ||
    hookLower.includes("lie")
  ) {
    return pickGrammar([
      "truth-stamp",
      "statement-lineup",
  ...
  if (
    hookLower.includes("poll") ||
    hookLower.includes("vote") ||
    hookLower.includes("opinion")
  ) {
    return pickGrammar([
      "hand-raising",
      "pie-chart-hero",
  ...
  if (
    hookLower.includes("math") ||
    hookLower.includes("calculate") ||
    hookLower.includes("solve") ||
    hookLower.includes("problem")
  ) {
    return pickGrammar([
      "chalkboard-equation",
      "equation-card",
  ...
  if (
    hookLower.includes("challenge") ||
    hookLower.includes("test") ||
    hookLower.includes("skill") ||
    hookLower.includes("limit")
  ) {
    return pickGrammar([
      "boss-hp-bar",
      "progress-ring-fire",
  ...
  if (
    hookLower.includes("game") ||
    hookLower.includes("play") ||
    hookLower.includes("controller") ||
    hookLower.includes("boss")
  ) {
    return pickGrammar([
      "boss-hp-bar",
      "game-over-glitch",
  ...
  if (
    hookLower.includes("brain") ||
    hookLower.includes("mind") ||
    hookLower.includes("iq")
  ) {
    return pickGrammar([
      "grid-faces",
      "brain-network",
  ...
  if (
    hookLower.includes("story") ||
    hookLower.includes("tale") ||
    hookLower.includes("chapter")
  ) {
    return pickGrammar([
      "book-cover",
      "typewriter-page",
 ...
 */
export function generateUniqueThumbnail(
  comp: Composition,
  seed: string,
  index = 0,
  excludeGrammars: string[] = [],
  ...
  const { text: hook, specifics } = extractHook(comp);
  const grammar = chooseGrammar(
    `${hook} ${comp.category}`,
    rng,
    excludeGrammars,
    index,
  ...
  const palette =
    PALETTES[(rng.int(0, PALETTES.length - 1) + index * 3) % PALETTES.length];
  const html = generateThumbnailHTML(
    grammar,
    hook,
    specifics,
    comp,
 ...
 * `specifics` carries real extracted content — every generator below that CAN
 * use it now does, falling back to generic content only when the field is absent.
 */
function generateThumbnailHTML(
  ...
  switch (grammar) {
    case "giant-question-mark":
      return generateRiddleSpotlight(
        headline,
        palette,
        specifics.riddleQuestion,
      );
    case "riddle-scroll":
      return generateRiddleScroll(headline, palette, specifics.riddleQuestion);
    case "grid-faces":
      return generateFaceGrid(shortHeadline, palette, rng);
    case "sequence-beads":
      return generateSequenceBeads(headline, palette, rng, specifics.sequence);
    case "chalkboard-equation":
      return generateChalkboard(headline, palette, rng, specifics.equation);
    case "equation-card":
      return generateEquationCard(
        headline,
        palette,
        specifics.equation,
        specifics.numMathProblems,
      );
    case "boss-hp-bar":
    ...
    case "truth-stamp":
      return generateTruthStamp(
        headline,
        palette,
        numScenes,
        specifics.statements,
        specifics.trueIndex,
      );
    case "statement-lineup":
      return generateStatementLineup(
        headline,
        palette,
        specifics.statements,
        specifics.trueIndex,
    ...
    case "red-x-stamp":
      return generateRedXStamp(headline, palette);
    case "neon-sign":
    ...
    case "hook-banner":
      return generateHookBanner(hook, palette, rng);
    ...
    case "numbered-sequence":
      return generateNumberedSequence(
        headline,
        palette,
        rng,
        specifics.sequence,
    ...
    case "blueprint-grid":
      return generateBlueprintGrid(headline, palette, specifics.equation);
    case "abacus-vintage":
    ...
    default:
      // Ultimate fallback now shows the actual hook text, not a generic template.
      return generateHookBanner(hook, palette, rng);
  }
}

// ============ NEW: functions that render REAL extracted content ============

function generateRiddleSpotlight(
  headline: string,
  p: any,
  riddleQuestion?: string,
): string {
  const sub = riddleQuestion ? riddleQuestion.substring(0, 90) : `${headline}`;
  return `<!DOCTYPE html>
...
.qmark{position:absolute;top:50%;left:50%;transform:translate(-50%,-60%);font-size:420px;font-weight:900;color:${p.primary};font-family:'Anton',sans-serif;text-shadow:0 30px 80px rgba(0,0,0,0.8),0 0 100px ${p.primary}80;opacity:0.95}
...
.headline{position:absolute;top:60px;left:70px;right:70px;font-size:34px;font-weight:700;color:${p.text};line-height:1.4;background:rgba(0,0,0,0.35);padding:24px 30px;border-radius:14px}
.tag{position:absolute;bottom:60px;left:70px;right:70px;font-family:'Anton',sans-serif;font-size:64px;color:${p.accent};text-align:center;text-shadow:4px 4px 0 ${p.bg}}
</style></head><body>
<div class="spotlight"></div>
<div class="circle"></div>
<div class="qmark">?</div>
<div class="headline">"${sub}${riddleQuestion && riddleQuestion.length > 90 ? "…" : ""}"</div>
<div class="tag">CAN YOU SOLVE IT?</div>
</body></html>`;
}

// A dedicated composition for when we HAVE the real riddle text and want it to
// be the actual hero, not a caption — an unrolled scroll with the question on it.
function generateRiddleScroll(
  headline: string,
  p: any,
  riddleQuestion?: string,
): string {
  const q = riddleQuestion || headline;
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@1,600&family=Cinzel:wght@700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
.scroll{width:960px;background:#efe4c8;border-radius:60px/24px;box-shadow:0 30px 80px rgba(0,0,0,0.6);padding:70px 90px;position:relative}
.scroll::before,.scroll::after{content:'';position:absolute;left:-20px;right:-20px;height:48px;background:#d8c79f;border-radius:50%;box-shadow:0 8px 16px rgba(0,0,0,0.3)}
.scroll::before{top:-24px}
.scroll::after{bottom:-24px}
.eyebrow{font-family:'Cinzel',serif;font-weight:700;font-size:26px;letter-spacing:0.3em;color:${p.accent};text-align:center;margin-bottom:30px}
.q{font-family:'EB Garamond',serif;font-style:italic;font-weight:600;font-size:56px;color:#2a2210;line-height:1.4;text-align:center}
</style></head><body>
<div class="scroll">
<div class="eyebrow">THE RIDDLE</div>
<div class="q">"${q}"</div>
</div>
</body></html>`;
}

function generateSequenceBeads(
  headline: string,
  p: any,
  rng: RNG,
  sequence?: number[],
  ...
  const seq =
    sequence && sequence.length > 0
      ? sequence
      : Array.from({ length: 6 }, () => rng.int(1, 9));
  const hiddenIdx = Math.floor(seq.length / 2);
  const beads = seq
    .map((n, i) => {
      const x = i * 190 + 100;
      const hidden = i === hiddenIdx;
      return `<g transform="translate(${x},260)">
<circle r="80" fill="${hidden ? p.accent : p.surface}" stroke="${p.primary}" stroke-width="6"/>
<text x="0" y="30" font-size="72" fill="${hidden ? p.bg : p.text}" text-anchor="middle" font-weight="900">${hidden ? "?" : n}</text>
</g>`;
    })
    .join("");
  return `<!DOCTYPE html>
...
.headline{position:absolute;top:60px;left:60px;right:60px;font-family:'Baloo 2',sans-serif;font-weight:800;font-size:60px;color:${p.text};text-align:center}
.sub{position:absolute;bottom:70px;left:60px;right:60px;font-family:'Baloo 2',sans-serif;font-weight:800;font-size:32px;color:${p.muted};text-align:center;letter-spacing:0.1em}
</style></head><body>
<div class="headline">${seq.length} IN A ROW</div>
<svg width="1280" height="620" font-family="Arial,sans-serif">${beads}</svg>
<div class="sub">WHAT'S MISSING?</div>
</body></html>`;
}
...
function generateChalkboard(
  ...
  const eq =
    realEquation ||
    rng.pick(["x² + 7x = 200", "π × r² = ?", "∑(1→n) = ?", "√(x+y) = z"]);
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
.chalk-eq{position:absolute;top:120px;left:100px;font-family:'Caveat',cursive;font-weight:700;font-size:${eq.length > 12 ? 96 : 140}px;color:#f2efe4;transform:rotate(-2.5deg);line-height:1;max-width:700px}
...
// A math variant that leads with the equation as the sole hero — no photo, no
// narrative, for videos where the numbers themselves are the whole hook.
function generateEquationCard(
  headline: string,
  p: any,
  equation?: string,
  numProblems?: number,
): string {
  const eq = equation || headline;
  const badge = numProblems ? `${numProblems} PROBLEMS` : "SOLVE THIS";
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:1280px;height:720px;background:radial-gradient(ellipse 1000px 800px at 30% 30%,${p.surface},${p.bg} 75%);display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.card{background:${p.bg};border:6px solid ${p.primary};border-radius:24px;padding:70px 90px;box-shadow:0 30px 70px rgba(0,0,0,0.5)}
.eq{font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:104px;color:${p.text};letter-spacing:-0.02em}
.badge{position:absolute;top:60px;right:70px;font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:26px;letter-spacing:0.1em;color:${p.bg};background:${p.accent};padding:14px 26px;border-radius:100px;transform:rotate(-4deg)}
</style></head><body>
<div class="badge">${badge}</div>
<div class="card"><div class="eq">${eq}</div></div>
</body></html>`;
}

function generateTruthStamp(
  headline: string,
  p: any,
  numFacts: number,
  realStatements?: string[],
  trueIndex?: number,
): string {
  const fallback = [
  ...
  const statements =
    realStatements && realStatements.length > 0
      ? realStatements.map((s) => `"${s}"`)
      : fallback;
  const tIdx = trueIndex ?? 2;
  const rows = statements
    .slice(0, Math.min(5, statements.length))
    .map((s, i) => {
      const isTrue = i === tIdx;
...
.strip .t{font-family:'Poppins',sans-serif;font-weight:700;font-size:34px;color:#17130f;flex:1}
...
// Distinct from truth-stamp: a plain vertical lineup with the "real" option
// underlined rather than highlighted, for would-you-rather / two-statement hooks.
function generateStatementLineup(
  headline: string,
  p: any,
  statements?: string[],
  trueIndex?: number,
  ...
  const list =
    statements && statements.length > 0
      ? statements
      : [headline, "...or something else entirely?"];
  const rows = list
    .slice(0, 4)
    .map((s, i) => {
      const marked = i === (trueIndex ?? 0);
      return `<div class="row"><span class="letter">${String.fromCharCode(65 + i)}</span><span class="t${marked ? " marked" : ""}">${s}</span></div>`;
    })
    .join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:1280px;height:720px;background:${p.bg};padding:80px 100px;font-family:'Libre Franklin',sans-serif}
.row{display:flex;align-items:center;gap:34px;margin-bottom:44px}
.letter{width:90px;height:90px;border-radius:50%;background:${p.surface};border:4px solid ${p.primary};display:flex;align-items:center;justify-content:center;font-weight:800;font-size:44px;color:${p.text};flex-shrink:0}
.t{font-weight:800;font-size:46px;color:${p.text};line-height:1.3}
.t.marked{color:${p.accent};text-decoration:underline;text-decoration-thickness:6px;text-underline-offset:10px}
</style></head><body>
${rows}
</body></html>`;
}

function generateRedXStamp(headline: string, p: any): string {
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
*{margin:0;padding:0;box-sizing:border-box}
body{width:1280px;height:720px;background:${p.bg};display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.x{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:600px;height:600px;opacity:0.85}
.headline{position:absolute;bottom:80px;left:80px;right:80px;font-family:'Anton',sans-serif;font-size:88px;color:${p.text};text-align:center;-webkit-text-stroke:6px ${p.bg};paint-order:stroke fill}
</style></head><body>
<svg class="x" viewBox="0 0 600 600"><line x1="60" y1="60" x2="540" y2="540" stroke="#ff2323" stroke-width="60" stroke-linecap="round"/><line x1="540" y1="60" x2="60" y2="540" stroke="#ff2323" stroke-width="60" stroke-linecap="round"/></svg>
<div class="headline">MYTH</div>
</body></html>`;
}

// Ultimate fallback: renders the actual extracted hook sentence in a plain,
// confident layout instead of a generic unrelated template.
function generateHookBanner(hook: string, p: any, rng: RNG): string {
  const rot = rng.int(-2, 2);
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Sora:wght@800&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:1280px;height:720px;background:radial-gradient(ellipse 1100px 900px at 70% 30%,${p.surface},${p.bg} 75%);display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.card{max-width:1000px;transform:rotate(${rot}deg);background:${p.bg};border:5px solid ${p.accent};border-radius:20px;padding:70px 80px;box-shadow:0 30px 70px rgba(0,0,0,0.5)}
.text{font-family:'Sora',sans-serif;font-weight:800;font-size:58px;color:${p.text};line-height:1.35}
</style></head><body>
<div class="card"><div class="text">${hook}</div></div>
</body></html>`;
}

// ============ EXISTING GENERATORS (unchanged, kept in full) ============
...
function generateFaceGrid(headline: string, p: any, rng: RNG): string {
  ...
  const faces = skinTones
    .map(
      (c) =>
        `<div class="cell"><svg width="100%" height="100%" viewBox="0 0 90 110"><ellipse cx="45" cy="50" rx="34" ry="42" fill="${c}"/><circle cx="32" cy="42" r="5" fill="#2a1f14"/><circle cx="58" cy="42" r="5" fill="#2a1f14"/><path d="M32 66 Q45 74 58 66" stroke="#2a1f14" stroke-width="3" fill="none" stroke-linecap="round"/></svg></div>`,
    )
    .join("");
...
<div class="polaroid"><div class="photo-inner"></div></div>
...
<svg class="circle" width="500" height="500" viewBox="0 0 500 500"><path d="M 450 250 A 200 200 0 1 1 50 250" fill="none" stroke="${p.accent}" stroke-width="40" stroke-linecap="round"/></svg>
...
<svg width="1280" height="720"><polyline points="${spiralPoints}" fill="none" stroke="${p.primary}" stroke-width="4" opacity="0.8"/><circle cx="640" cy="360" r="8" fill="${p.accent}"/></svg>
...
<div class="track track1"></div><div class="track track2"></div>
...
<svg class="beacon" width="60" height="60" viewBox="0 0 60 60"><circle cx="30" cy="30" r="25" fill="${p.accent}" opacity="0.9"/><circle cx="30" cy="30" r="15" fill="#fff"/></svg>
<div class="pulse"></div>
...
<div class="content"><div class="classified">CLASSIFIED</div><div>SUBJECT:</div><div class="subject">${headline}</div></div>
</div>
</body></html>`;
}
...
function generateNumberedSequence(
  ...
  const seq =
    realSequence && realSequence.length >= 6
      ? realSequence.slice(0, 9)
      : Array.from({ length: 9 }, (_, i) => i + 1);
  const missingAt = Math.floor(seq.length / 2);
  const numbers = seq
    .map((n, i) => {
      const x = (i % 3) * 300 + 240;
      const y = Math.floor(i / 3) * 200 + 160;
      const isMissing = i === missingAt;
      return `<g transform="translate(${x},${y})"><rect x="-100" y="-80" width="200" height="160" rx="20" fill="${isMissing ? p.accent : p.surface}" stroke="${p.primary}" stroke-width="4"/><text x="0" y="20" font-size="${isMissing ? "80" : "90"}" fill="${isMissing ? p.bg : p.text}" text-anchor="middle" font-weight="900">${isMissing ? "?" : n}</text></g>`;
    })
    .join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
function generatePolaroidScatter(headline: string, p: any, rng: RNG): string {
  const photos = Array.from({ length: 6 }, (_, i) => {
    ...
    return `<g transform="translate(${x},${y}) rotate(${rot})"><rect x="-80" y="-100" width="160" height="200" fill="#fff" stroke="#ddd" stroke-width="2" rx="4"/><rect x="-70" y="-90" width="140" height="140" fill="${p.surface}"/><text x="0" y="70" font-size="24" fill="#666" text-anchor="middle">${i + 1}</text></g>`;
  }).join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
<div class="stack"><div class="card card1">?</div><div class="card card2">?</div><div class="card card3">7</div></div>
</body></html>`;
}
...
<div class="calc"><div class="display">42</div>
<div class="buttons"><div class="btn">7</div><div class="btn">8</div><div class="btn">9</div><div class="btn" style="background:${p.accent}">÷</div><div class="btn">4</div><div class="btn">5</div><div class="btn">6</div><div class="btn" style="background:${p.accent}">×</div></div>
</div>
</body></html>`;
}
...
function generateBlueprintGrid(
  headline: string,
  p: any,
  realEquation?: string,
): string {
  const gridLines = Array.from(
    { length: 15 },
    (_, i) =>
      `<line x1="0" y1="${i * 50}" x2="1280" y2="${i * 50}" stroke="rgba(255,255,255,0.2)" stroke-width="1"/>`,
  )
    .concat(
      Array.from(
        { length: 26 },
        (_, i) =>
          `<line x1="${i * 50}" y1="0" x2="${i * 50}" y2="720" stroke="rgba(255,255,255,0.2)" stroke-width="1"/>`,
      ),
    )
    .join("");
  const eq = realEquation || "42 ÷ 6 = ?";
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
.equation{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-family:'Orbitron',sans-serif;font-weight:700;font-size:${eq.length > 12 ? 64 : 96}px;color:#fff;text-align:center;background:rgba(10,37,64,0.9);padding:60px;border:4px solid ${p.accent};box-shadow:0 0 40px ${p.accent}}
...
function generateAbacusVintage(headline: string, p: any, rng: RNG): string {
  ...
  const rods = Array.from(
    { length: 7 },
    (_, i) =>
      `<line x1="180" y1="${120 + i * 80}" x2="1100" y2="${120 + i * 80}" stroke="${p.muted}" stroke-width="6"/>`,
  ).join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
<div class="frame"><svg width="1280" height="720">${rods}${beads}</svg></div>
...
<div class="paper">
<div class="lines"></div>
<svg width="1280" height="720"><circle cx="640" cy="360" r="200" fill="none" stroke="${p.primary}" stroke-width="4"/><line x1="440" y1="360" x2="840" y2="360" stroke="${p.accent}" stroke-width="4"/><circle cx="640" cy="360" r="8" fill="${p.accent}"/><text x="640" y="180" font-size="36" fill="${p.muted}" text-anchor="middle" font-family="Georgia">r</text></svg>
...
<div class="display"><div class="time">12:34:56</div></div>
...
function generateBarGraphRace(headline: string, p: any, rng: RNG): string {
  const bars = Array.from({ length: 5 }, (_, i) => {
    ...
    const y = 600 - height;
    const x = i * 200 + 140;
    return `<rect x="${x}" y="${y}" width="120" height="${height}" fill="${i === 0 ? p.accent : p.primary}" rx="10"/><text x="${x + 60}" y="${y - 20}" font-size="48" fill="${p.text}" text-anchor="middle" font-weight="900">${rng.int(45, 95)}%</text>`;
  }).join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
function generateCrowdSilhouettes(headline: string, p: any, rng: RNG): string {
  const crowd = Array.from({ length: 20 }, (_, i) => {
    ...
    return `<rect x="${x}" y="${720 - height}" width="60" height="${height}" fill="${p.primary}" opacity="${rng.next() * 0.4 + 0.6}"/><circle cx="${x + 30}" cy="${720 - height - 15}" r="25" fill="${p.primary}" opacity="${rng.next() * 0.4 + 0.6}"/>`;
  }).join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
...
<g transform="translate(300,360)"><rect x="-80" y="-100" width="160" height="200" rx="40" fill="${p.accent}"/><rect x="-60" y="100" width="50" height="80" rx="15" fill="${p.accent}"/><rect x="-100" y="-120" width="80" height="40" rx="20" fill="${p.accent}"/></g>
<g transform="translate(980,360) scale(1,-1)"><rect x="-80" y="-100" width="160" height="200" rx="40" fill="${p.primary}"/><rect x="-60" y="100" width="50" height="80" rx="15" fill="${p.primary}"/><rect x="-100" y="-120" width="80" height="40" rx="20" fill="${p.primary}"/></g>
</svg>
<div class="vs">VS</div>
...
<defs><linearGradient id="fireGrad" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stop-color="#ff4400" stop-opacity="0.9"/><stop offset="100%" stop-color="#ff0000" stop-opacity="0.3"/></linearGradient></defs>
</svg>
...
<svg class="cabinet" viewBox="0 0 600 650"><path d="M 100 0 L 50 550 L 100 650 L 500 650 L 550 550 L 500 0 Z" fill="${p.surface}" stroke="#000" stroke-width="8"/></svg>
<div class="screen"><div class="title">PLAY!</div></div>
...
<div class="clouds">${Array.from({ length: 3 }, (_, i) => `<ellipse cx="${200 + i * 400}" cy="${80 + i * 30}" rx="100" ry="40" fill="#fff" opacity="0.8"/>`).join("")}</div>
...
<div class="lights">${Array.from({ length: 10 }, (_, i) => `<circle cx="${i * 128 + 64}" cy="50" r="30" fill="#ffd700" opacity="${(i % 2) * 0.5 + 0.5}"/>`).join("")}</div>
<div class="stage"></div>
...
<div class="buzzer"><div class="label">BUZZ!</div></div>
</body></html>`;
}
...
<div class="book"><div class="ornament"></div><div class="title">${headline}</div><div class="ornament"></div><div class="author">A Story</div></div>
</body></html>`;
}
...
<div class="paper"><div class="lines"></div><div class="text"><div class="title">${headline}</div>Once upon a time...<span class="cursor"></span></div></div>
</body></html>`;
}
...
function generateFilmStrip(headline: string, p: any, rng: RNG): string {
  const frames = Array.from({ length: 4 }, (_, i) => {
    const x = i * 280 + 100;
    return `<g transform="translate(${x},200)"><rect width="240" height="280" fill="${p.surface}" stroke="${p.primary}" stroke-width="4" rx="8"/><rect x="10" y="10" width="220" height="220" fill="${p.bg}"/><text x="120" y="270" font-size="24" fill="${p.text}" text-anchor="middle">${i + 1}</text></g>`;
  }).join("");
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
...
<div class="headline">${headline}</div>
<svg width="1280" height="720">${frames}${Array.from({ length: 10 }, (_, i) => `<circle cx="50" cy="${i * 72 + 36}" r="15" fill="${p.accent}"/><circle cx="1230" cy="${i * 72 + 36}" r="15" fill="${p.accent}"/>`).join("")}</svg>
</body></html>`;
}
...
<div class="book"><div class="gutter"></div><div class="illustration"></div><div class="text">${headline}</div></div>
</body></html>`;
}
...
<svg width="300" height="50" viewBox="0 0 300 50" style="margin-bottom:20px"><path d="M 0 25 Q 75 5 150 25 T 300 25" fill="none" stroke="${p.accent}" stroke-width="2"/><circle cx="150" cy="25" r="8" fill="${p.accent}"/></svg>
<div class="title">${headline}</div>
<svg width="300" height="50" viewBox="0 0 300 50"><path d="M 0 25 Q 75 45 150 25 T 300 25" fill="none" stroke="${p.accent}" stroke-width="2"/><circle cx="150" cy="25" r="8" fill="${p.accent}"/></svg>
</div>
</body></html>`;
}
...
.tag{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-5deg);font-family:'Permanent Marker',cursive;font-size:160px;color:${p.accent};text-shadow:8px 8px 0 ${p.primary},-4px -4px 0 ${p.bg},0 0 40px ${p.accent}}
</style></head><body>
<div class="brick"></div>
...
<svg width="1280" height="720">${Array.from({ length: 100 }, () => {
    const x = Math.random() * 1280;
    const y = Math.random() * 720;
    return `<line x1="${x}" y1="${y}" x2="${x}" y2="${y + 20}" stroke="rgba(255,255,255,0.2)" stroke-width="1"/>`;
  }).join("")}</svg>
<div class="billboard"><div class="text">${headline}</div></div>
</body></html>`;
}
...
<div class="magazine"><div class="masthead">FEATURED</div><div class="cover-story">${headline}</div><div class="barcode"></div></div>
</body></html>`;
}
...
<div class="ticket"><div class="perforation"></div><div class="stub"><div class="admit">ADMIT ONE</div></div><div class="event">${headline}</div></div>
</body></html>`;
}
...
<div class="receipt"><div class="store">*** RECEIPT ***</div><div class="item">${headline}</div><div class="item">Qty: 1</div><div class="total">PAID</div></div>
</body></html>`;
}
